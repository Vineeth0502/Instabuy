...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  passport.deserializeUser(async (id: number | string, done: (err: any, user?: Express.User | false) => void) => {
    try {
      ...
      // If the id is a MongoDB ObjectId (string), we need to find the user by _id
      let user;
      ...
      // No need for conversion as storage.getUser already returns a plain object via toObject()
